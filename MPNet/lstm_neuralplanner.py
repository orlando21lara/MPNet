...
def getPointCloud(env_idx):
    obs = []
    temp=np.fromfile('dataset/obs_cloud/obc'+str(env_idx)+'.dat')
    obs.append(temp)
...
def run_models():
    random_configs = 10
    num_envs = 10    # Value should not exceed 10

    mlp_path_lengths    = np.zeros((num_envs, random_configs), dtype=np.float32)
    mlp_plan_times      = np.zeros((num_envs, random_configs), dtype=np.float32)
    mlp_feasible_paths  = np.zeros((num_envs, random_configs), dtype=np.int8)
    mlp_path_sizes      = np.zeros((num_envs, random_configs), dtype=np.int32)

    lstm_mlp_path_lengths   = np.zeros((num_envs, random_configs), dtype=np.float32)
    lstm_mlp_plan_times     = np.zeros((num_envs, random_configs), dtype=np.float32)
    lstm_mlp_feasible_paths = np.zeros((num_envs, random_configs), dtype=np.int8)
    lstm_mlp_path_sizes     = np.zeros((num_envs, random_configs), dtype=np.int32)

    # Iterate through the last 10 environments. Starting from environment 100 is unseen
    print("#####################################")
    print("Running models on unseen environments")
    for env_idx in tqdm(range(num_envs), position=0):
        environment_padding = 1.0

        # Get the environment bounds based off the obstacle point cloud
        obs = getPointCloud(env_idx)
        x_min = np.min(obs[:,0]) - environment_padding
        x_max = np.max(obs[:,0]) + environment_padding
        y_min = np.min(obs[:,1]) - environment_padding
        y_max = np.max(obs[:,1]) + environment_padding

        # Iterate through 10 random start and goal positions
        for i in tqdm(range(random_configs), position=1, leave=False):
            # Generate random start and goal positions
            # intialize start to be of type float32
            start = np.random.uniform(low=[x_min, y_min], high=[x_max, y_max]).astype(np.float32)
            goal = np.random.uniform(low=[x_min, y_min], high=[x_max, y_max]).astype(np.float32)

            # Generate path using MLP
            mlp_path, mlp_plan_time, mlp_feasible = generate_path(env_idx, start, goal, mlp)

            mlp_path_lengths[env_idx][i] = get_path_length(mlp_path)
            mlp_path_sizes[env_idx][i] = len(mlp_path)
            mlp_plan_times[env_idx][i] = mlp_plan_time
            mlp_feasible_paths[env_idx][i] = mlp_feasible
            save_path(mlp_path, "mlp_e" + str(env_idx) + "_path" + str(i))

            # Generate path using LSTM + MLP
            lstm_mlp_path, lstm_plan_time, lstm_feasible = generate_path(env_idx, start, goal, lstm_mlp)

            lstm_mlp_path_lengths[env_idx][i] = get_path_length(lstm_mlp_path)
            lstm_mlp_path_sizes[env_idx][i] = len(lstm_mlp_path)
            lstm_mlp_plan_times[env_idx][i] = lstm_plan_time
            lstm_mlp_feasible_paths[env_idx][i] = lstm_feasible
            save_path(lstm_mlp_path, "lstm_mlp_e" + str(env_idx) + "_path" + str(i))
    
    # Save results
    np.save("results/stats/mlp_path_lengths.npy",           mlp_path_lengths)
    np.save("results/stats/mlp_plan_times.npy",             mlp_plan_times)
    np.save("results/stats/mlp_feasible_paths.npy",         mlp_feasible_paths)
    np.save("results/stats/mlp_path_sizes.npy",             mlp_path_sizes)
    np.save("results/stats/lstm_mlp_path_lengths.npy",      lstm_mlp_path_lengths)
    np.save("results/stats/lstm_mlp_plan_times.npy",        lstm_mlp_plan_times)
    np.save("results/stats/lstm_mlp_feasible_paths.npy",    lstm_mlp_feasible_paths)
    np.save("results/stats/lstm_mlp_path_sizes.npy",        lstm_mlp_path_sizes)

def compare_models():
    # Load the stats
    mlp_path_lengths        = np.load("results/stats/mlp_path_lengths.npy")
    mlp_plan_times          = np.load("results/stats/mlp_plan_times.npy")
    mlp_feasible_paths      = np.load("results/stats/mlp_feasible_paths.npy")
    mlp_path_sizes          = np.load("results/stats/mlp_path_sizes.npy")

    lstm_mlp_path_lengths   = np.load("results/stats/lstm_mlp_path_lengths.npy")
    lstm_mlp_plan_times     = np.load("results/stats/lstm_mlp_plan_times.npy")
    lstm_mlp_feasible_paths = np.load("results/stats/lstm_mlp_feasible_paths.npy")
    lstm_mlp_path_sizes     = np.load("results/stats/lstm_mlp_path_sizes.npy")

    # Print stats
    print("\nMLP:")
    print("\t Avg. path lengths: ", np.mean(mlp_path_lengths))
    print("\t Avg. path sizes: ", np.mean(mlp_path_sizes))
    print("\t Avg. plan times: ", np.mean(mlp_plan_times))
    print("\t Num. feasible paths: ", np.sum(mlp_feasible_paths))

    print("\nLSTM + MLP:")
    print("\t Avg. path lengths: ", np.mean(lstm_mlp_path_lengths))
    print("\t Avg. path sizes: ", np.mean(lstm_mlp_path_sizes))
